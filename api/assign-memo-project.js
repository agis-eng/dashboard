...
// POST { memoId, project } — updates project_match in voice_memos.yaml via GitHub API
// Uses same ghGet/ghPut pattern as update-field.js so it works on Vercel.
...
export default async function handler(req, res) {
  ...
  try {
    ...
    const result = await ghPut(
      ...
      newYaml,
      sha,
      `dashboard: assign memo "${memoId}" to project "${project || 'none'}"`
    );
    ...
    console.error('assign-memo-project error:', err);
