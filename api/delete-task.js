...
export default async function handler(req, res) {
  ...
  try {
    ...
    const result = await ghPut('data/tasks.yaml', newYaml, sha, `dashboard: delete task "${deletedTask.title}"`);

    if (result.content) {
      return res.status(200).json({ ok: true, taskId, title: deletedTask.title });
    } else {
